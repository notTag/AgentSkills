---
name: blood-results-analysis
description: >
  Structured analysis of Rythm Health (or comparable) blood-work panels uploaded as PDF
  or CSV. Runs a conversational intake interview first (training history, diet, sleep,
  goals, symptoms, medications), then loads the lab files and delivers a context-grounded
  report covering wins, out-of-range markers, suboptimal-but-in-range markers, lifestyle
  confounds, drug/supplement interactions, follow-up tests, prioritized action items, and
  physician discussion points. Honors long-training-history context (e.g., elevated
  creatinine/hematocrit/ferritin in heavy lifters) before flagging concerns. Never gives
  medical advice or prescription dosing. Trigger when the user (1) uploads a blood-panel
  PDF or CSV, (2) asks to "analyze my blood work / labs / panel / Rythm results",
  (3) types "/blood-results", "/bloodwork", or "/labs", or (4) pastes blood marker values
  asking for interpretation.
---

# Blood Results Analysis Skill

> **Purpose:** Reusable workflow for analyzing Rythm Health blood work results (PDF or CSV). On invocation, conduct a structured intake interview, then deliver a personalized lab analysis grounded in the user's training history, lifestyle, and goals.
...
Ask the user the following questions. Use the `AskUserQuestion` tool where it makes sense (single-select / multi-select), and ask free-text questions one at a time so the user is not overwhelmed. Adapt phrasing to be conversational, not clinical.
...
- If the file is a **CSV**, expect columns: `marker, value, unit, reference_range, status, time`. Use `Read` to load it.

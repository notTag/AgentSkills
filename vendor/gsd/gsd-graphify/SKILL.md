...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node $HOME/.claude-wakelax/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node $HOME/.claude-wakelax/get-shit-done/bin/gsd-tools.cjs config-set graphify.enabled true
...
```bash
node $HOME/.claude-wakelax/get-shit-done/bin/gsd-tools.cjs graphify query <term>
```
...
```bash
node $HOME/.claude-wakelax/get-shit-done/bin/gsd-tools.cjs graphify status
```
...
```bash
node $HOME/.claude-wakelax/get-shit-done/bin/gsd-tools.cjs graphify diff
```
...
```bash
node "$HOME/.claude-wakelax/get-shit-done/bin/gsd-tools.cjs" graphify build
```
...
graphify update . \
  ...
  && node "$HOME/.claude-wakelax/get-shit-done/bin/gsd-tools.cjs" graphify build snapshot \
  && node "$HOME/.claude-wakelax/get-shit-done/bin/gsd-tools.cjs" graphify status
```
